...
- **ケースメモリ（使うほど賢くなる）**：レポート承認済みの案件を事例として保存し、次回の類似案件の分析時に自動参照します（Case-Based Reasoning）。保存するのは設備プロファイル（担当者の入力）と法令名・優先度のみで、適用理由・総括などのLLM生成テキストは保存しません（Google検索グラウンディング由来の情報を長期保存しないための Gemini API 規約対応）。
...
- **いつでもAIに質問（QA）**：画面右上の「💬 AIに質問」から、ヒアリング中・方針確認・結果確認・レポート確認・完了後のどの段階でも、専門用語や判断理由などわからないことをその場でAIに質問できます。質問しても案件は進まず（ヒアリング回答・承認・再調査は従来どおり画面内の入力欄・ボタンのみ）、回答は案件の設備情報・調査方針・調査結果に即して質問パネル内に表示されます（参考情報。メイン画面の会話履歴には混ざりません）。
- **AI利用コストの見える化**：Langfuse 設定時はコスト計測を Langfuse に一本化し、サイドバーには Langfuse が計算した**実測コスト**を案件（セッション）単位で取得・表示します（「🔄 コスト表示を更新」で再取得。アプリ内での概算計算は行いません）。Langfuse 未設定の場合は、確かな単価が分かる呼び出しのみサイドバーで概算表示し、単価不明分は金額に含めず明示します。
