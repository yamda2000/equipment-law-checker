...
- **漏れ防止の工夫**：ルールベース必須法令の決定的シード検索、主要11法令領域の「該当/非該当（理由つき）」強制振り分け、論点ごとの網羅性検証、不明・未定情報の明示。
...
│       ├── web_search.py       Gemini Google Search Grounding ＋ 公式ページ本文取得
...
| `LLM_COST_INPUT_PER_1M` ほか | コスト表示の単価設定（詳細は `.env.example` 参照） | 任意 |
