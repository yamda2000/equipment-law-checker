...
## 保存内容の制限（Gemini API 追加利用規約対応）
適用理由・総括・届出/対応項目などの LLM 生成テキストには、Google 検索
グラウンディング由来の知見が間接的に含まれうるため、事例には保存しない。
保存するのは以下のみ:
- 設備プロファイル（ヒアリングでの担当者入力＝Gemini と無関係）
...
def save_case(
    ...
    法令名・優先度以外の LLM 生成テキストは保存しない（モジュール先頭の
    保存内容の制限を参照）。"""
    ...
    record = {
        ...
        "excluded_laws": [
            e.get("law_name", "") if isinstance(e, dict) else str(e)
...
def format_cases_for_prompt(cases: list[dict]) -> str:
    ...
    保存しているのは法令名と優先度だけなので、確認漏れ防止のチェックリスト
    として提示する（適用理由・対応内容は保存していない）。"""
    if not cases:
        return ""
    blocks = []
    for i, c in enumerate(cases, 1):
        ...
        laws = c.get("laws", [])
        if laws:
            lines.append("【該当した法令（優先度）】")
            for l in laws:
                pr = _PRIORITY_JA.get(l.get("priority", ""), l.get("priority", ""))
                lines.append(f"- {l.get('law_name', '')}（{pr}）")
        excluded = [
            e.get("law_name", "") if isinstance(e, dict) else str(e)
            for e in c.get("excluded_laws", [])
        ]
