...
# ─── 台帳ファイルの読み書き ───────────────────────────────────────
# ※ _load_registry / _save_registry は _LOCK 保持中から呼ばれる。
#    threading.Lock は再入不可のため、これらの中でロックを取ってはならない
#    （公開関数の側でロックする）。
# Windows では置換対象を他プロセスが開いている間 os.replace が拒否される
# （WinError 5）。ごく短時間の保持がほとんどのため、少し待って再試行する
_REPLACE_RETRIES = 5
_REPLACE_WAIT    = 0.05   # 秒（0.05 → 0.10 → … と増やす）


def _atomic_write_json(path: Path, data) -> None:
    """JSON を原子的に書き出す（一時ファイル＋os.replace）。ケースメモリと共用。

    直接上書きすると、書き込み途中でプロセスが落ちた場合に壊れた JSON が
    残り、以後の読み取りが「登録なし（0件）」として静かに失敗する。
    同一ボリューム内の os.replace は置換が原子的なため、読み手は常に
    新旧どちらか一方の完全な内容を見る。
    """
    ...
    try:
        with open(tmp, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())   # OS クラッシュ時の内容欠損を防ぐ
        for attempt in range(_REPLACE_RETRIES):
            try:
                os.replace(tmp, path)
                break
            except PermissionError:
                # 他プロセス（別ワーカー・ウイルス対策・エディタ等）が
                # 対象ファイルを開いている間は置換できない。待って再試行する
...
def _load_json_ledger(path: Path, label: str) -> list[dict]:
    """台帳 JSON を読む。未作成なら空リスト（正常）。

    ファイルが存在するのに読めない場合は破損の可能性があるため、
    必ずログに残す（「まだ登録がない」状態と区別できないまま
    静かに 0 件を返すと、登録済み資料が無視されたことに気づけない）。
    """
    if not path.exists():
        return []
    try:
        ...
        logger.exception("%sを読み込めませんでした（破損の可能性）: %s", label, path)
        return []


def _load_registry() -> list[dict]:
    return _load_json_ledger(REGISTRY_PATH, "社内文書の台帳")
...
def list_registered() -> list[dict]:
    ...
    """
    with _LOCK:   # 登録・削除の書き込み中に読んで空リストにならないよう保護する
        reg = _load_registry()
    return [
